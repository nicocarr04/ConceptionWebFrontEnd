...
const useUser = () => {
    ...
    async function getUserRoles(id) {
        const resultat = await axios.get(`http://localhost:5000/users/${id}/roles`)
        return resultat.data

    }

    async function getUserNotes(id) {
        const resultat = await axios.get(`http://localhost:5000/users/${id}/notes`)
        return resultat.data

    }

    async function getUserMatieres(id) {
        const resultat = await axios.get(`http://localhost:5000/users/${id}/matieres`)
        return resultat.data
